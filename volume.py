...
# 日経225銘柄リスト（一部抜粋ではなく全225銘柄を対象にするのが効果的です）
...
for t in TICKERS:
    try:
        ...
        if current_vol > avg_vol * 2.5:
            ...
            volume_hits.append(f"🔥{t}: {price:.1f}円\n(通常比 {ratio:.1f}倍の爆増！)")
        
        time.sleep(0.05)
    except: continue

if volume_hits:
    send_line_message("【日経225・出来高急騰警報】\n\n" + "\n\n".join(volume_hits[:15]))
else:
    send_line_message("【日経225・出来高急騰】\n本日の急騰銘柄はありませんでした。")
